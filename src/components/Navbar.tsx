         ...
         {/* Desktop Nav */}
          <div className="hidden lg:flex items-center gap-0.5 xl:gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.key}
                to={link.path}
                className={`px-2.5 py-1.5 text-sm rounded-md transition-colors whitespace-nowrap ${
                  isActive(link.path)
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="xs:hidden">{language === "en" ? "தமிழ்" : "English"}</span>
            </Button>
            <button
              className="lg:hidden p-2 text-foreground rounded-md hover:bg-muted transition-colors touch-manipulation"
          ...
          <div className="lg:hidden py-3 border-t border-border grid grid-cols-2 gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-3 py-3 text-sm rounded-md transition-colors touch-manipulation ${
                  isActive(link.path)
                    ? "bg-primary text-primary-foreground"
